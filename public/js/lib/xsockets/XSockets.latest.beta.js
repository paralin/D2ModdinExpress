...
XSockets.BinaryMessage = (function () {
    ...
    function BinaryMessage(message, arrayBuffer, cb) {
        ...
        if (arguments.length === 3) {
            ...
            var result = (new XSockets.Message()).parse(ab2str(payload), buffer);
            arrayBuffer(result);
        }
    }
...
XSockets.Message = (function () {
    function message(topic, object, contoller) {
        this.T = topic;
        this.D = object;
        this.C = contoller;
        this.JSON = {
    ...
    return {
        getInstance: function (url, events, subprotocol,force) {
            if (!communicationInstance || force) {
                 communicationInstance = createCommunication(url.toLocaleLowerCase(), events, subprotocol);
            }
...
XSockets.Controller = (function () {
    ...
    instance.prototype.setProperty = function (name, value) {
        ...
        this.publish(new XSockets.Message(property, typeof (value) === "object" ? value : {
            value: value
        }, this.name));
    };
...
XSockets.WebSocket = (function () {
    function ctor(url, controllers, parameters) {
        ...
        controllers = controllers || [(this.uri.controller).toLocaleLowerCase()];
        ...
        this.controller = function (controller) {
            if (!self.hasOwnProperty(controller)) {
                throw "The specified contoller '" + controller + "' cannot be found. Known controllers are " + JSON.stringify(self.controllerInstances);
            }
